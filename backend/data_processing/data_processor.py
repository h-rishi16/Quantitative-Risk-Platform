...
class DataProcessor:
    ...
    def validate_portfolio_weights(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            logger.info(f"Portfolio weights validated: {len(df)} assets, sum={weight_sum:.4f}")
            return True
    ...
    def validate_historical_returns(self, df: pd.DataFrame, portfolio_assets: List[str]) -> bool:
        ...
        try:
            ...
            if len(df) < 30:
                logger.warning(f"Limited historical data: {len(df)} observations. Recommend at least 30.")
            
            logger.info(f"Historical returns validated: {len(df)} observations for {len(portfolio_assets)} assets")
            return True
    ...
    def get_portfolio_data(self) -> Optional[Dict]:
        ...
        try:
            ...
            weights_array = self.portfolio_weights.set_index('asset').loc[self.assets]['weight'].values
    ...
    def calculate_portfolio_statistics(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'correlation_range': [correlation_matrix.min(), correlation_matrix.max()],
