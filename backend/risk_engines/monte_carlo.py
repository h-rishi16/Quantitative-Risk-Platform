...
class MonteCarloVaR:
    ...
    def __init__(self, assets: Optional[List[AssetParameters]] = None, correlation_matrix: Optional[np.ndarray] = None):
        """
    ...
    def from_historical_data(cls,
        ...
        return cls.from_data(asset_names, weights, expected_returns, volatilities, correlation_matrix)
    ...
    def _validate_correlation_matrix(self, corr_matrix: np.ndarray) -> None:
        ...
        if corr_matrix.shape != (self.num_assets, self.num_assets):
            raise ValueError(f"Correlation matrix shape {corr_matrix.shape} doesn't match number of assets {self.num_assets}")
    ...
    def simulate_portfolio_returns(self, 
                                 ...
                                 process: StochasticProcess = StochasticProcess.GEOMETRIC_BROWNIAN_MOTION) -> np.ndarray:
        """
        ...
        random_numbers = self._generate_correlated_randoms(config.num_simulations, config.time_horizon)
        ...
        for i, asset in enumerate(self.assets):
            if process == StochasticProcess.GEOMETRIC_BROWNIAN_MOTION:
                asset_returns = self._simulate_gbm(asset, random_numbers[i], dt, config.time_horizon)
            elif process == StochasticProcess.MEAN_REVERSION:
                asset_returns = self._simulate_mean_reversion(asset, random_numbers[i], dt, config.time_horizon)
            else:
    ...
    def _generate_correlated_randoms(self, num_sims: int, time_horizon: int) -> np.ndarray:
        ...
        independent_randoms = np.random.normal(0, 1, (self.num_assets, num_sims, time_horizon))
        
        # Apply correlation structure
        correlated_randoms = np.zeros_like(independent_randoms)
        for sim in range(num_sims):
            for t in range(time_horizon):
                correlated_randoms[:, sim, t] = chol_matrix @ independent_randoms[:, sim, t]
        
        return correlated_randoms
    
    def _simulate_gbm(self, asset: AssetParameters, randoms: np.ndarray, dt: float, time_horizon: int) -> np.ndarray:
        """
    ...
    def _simulate_mean_reversion(self, asset: AssetParameters, randoms: np.ndarray, dt: float, time_horizon: int) -> np.ndarray:
        """
        ...
        for t in range(time_horizon):
            ...
            d_log_prices = theta * (np.log(asset.current_price) + mu - log_prices) * dt + \
                          asset.volatility * randoms[:, t] * np.sqrt(dt)
    ...
    def calculate_var(self, config: SimulationConfig, **kwargs) -> VaRResults:
        ...
        logger.info(f"VaR calculation completed for {len(config.confidence_levels)} confidence levels")
    ...
    def stress_test(self, 
                   ...
                   config: SimulationConfig) -> Dict[str, VaRResults]:
        """
    ...
    def generate_report(self, results: VaRResults, portfolio_value: float = 1000000) -> str:
        """
        ...
        for asset in self.assets:
            report.append(f"{asset.symbol}: {asset.weight:.1%} (μ={asset.expected_return:.2%}, σ={asset.volatility:.2%})")
        report.append("")
        ...
        report.append("-" * 30)
        report.append(f"{'Confidence':<12} {'VaR (%)':<10} {'VaR ($)':<15} {'CVaR (%)':<10} {'CVaR ($)':<15}")
        report.append("-" * 65)
        ...
        for conf_level in sorted(results.var_estimates.keys()):
            ...
            report.append(f"{conf_level:.1%}        {var_pct:.4f}    ${var_dollar:>12,.0f}   {cvar_pct:.4f}    ${cvar_dollar:>12,.0f}")
        
        report.append("")
        report.append("Notes:")
        report.append("- VaR represents the maximum expected loss at given confidence level")
        report.append("- CVaR (Conditional VaR) represents the expected loss beyond VaR threshold")
