...
async def calculate_monte_carlo_var(request: MonteCarloRequest):
    ...
    try:
        ...
        if len(weights) != len(request.assets):
            raise HTTPException(status_code=400, detail="Number of weights must match number of assets")
        
        if returns_matrix.shape[1] != len(request.assets):
            raise HTTPException(status_code=400, detail="Returns matrix columns must match number of assets")
...
async def calculate_var_from_csv(
    ...
    confidence_levels: str = Form("0.95,0.99", description="Comma-separated confidence levels"),
    ...
    try:
        ...
        if not portfolio_file.filename.endswith('.csv'):
            raise HTTPException(status_code=400, detail="Portfolio file must be CSV format")
        if not returns_file.filename.endswith('.csv'):
            raise HTTPException(status_code=400, detail="Returns file must be CSV format")
        ...
        if portfolio_df is None:
            raise HTTPException(status_code=400, detail="Invalid portfolio weights format")
        
        # Load historical returns
        returns_df = processor.load_historical_returns(returns_csv)
        if returns_df is None:
            raise HTTPException(status_code=400, detail="Invalid historical returns format")
        
        # Get processed portfolio data
        portfolio_data = processor.get_portfolio_data()
        if not portfolio_data:
            raise HTTPException(status_code=400, detail="Failed to process portfolio data")
        
        # Parse confidence levels
        try:
            conf_levels = [float(x.strip()) for x in confidence_levels.split(',')]
        except:
            raise HTTPException(status_code=400, detail="Invalid confidence levels format")
...
async def get_sample_data():
    ...
    try:
        # Read sample files
        portfolio_path = "/Users/hrishi/Projects/Risk Model/sample_data/portfolio_weights.csv"
        returns_path = "/Users/hrishi/Projects/Risk Model/sample_data/historical_returns.csv"
    ...
    try:
        ...
        if returns_valid:
            validation_result["returns_info"] = {
                ...
                "date_range": [str(returns_df['date'].min()), str(returns_df['date'].max())],
                "assets_with_data": [col for col in returns_df.columns if col != 'date']
            }
