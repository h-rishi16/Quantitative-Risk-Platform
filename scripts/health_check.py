...
class HealthChecker:
    ...
    def check_python_environment(self) -> bool:
        ...
        if missing_files:
            ...
            self.print_result("Project Structure", True, "All expected directories found")
            return True
    ...
    def check_dependencies(self) -> bool:
        ...
        core_packages = [
            ...
            'python_multipart', 'python_dotenv', 'aiofiles'
        ]
        
        # Development dependencies
        dev_packages = [
            'pytest', 'httpx', 'black', 'isort', 'flake8', 
            'bandit', 'safety', 'coverage'
        ]
    ...
    def check_backend_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.print_result("Health Endpoint", True, f"Response: {response.json()}")
            else:
                self.print_result("Health Endpoint", False, f"Status: {response.status_code}")
                return False
            ...
            if response.status_code == 200:
                data = response.json()
                self.print_result("Sample Data Endpoint", True, f"Assets: {len(data.get('portfolio_weights', []))}")
            else:
                self.print_result("Sample Data Endpoint", False, f"Status: {response.status_code}")
                return False
            ...
            response = requests.post(f"{self.backend_url}/monte_carlo_var", 
                                   json=test_payload, timeout=30)
            if response.status_code == 200:
                result = response.json()
                var_results = result.get('var_results', [])
                self.print_result("Monte Carlo API", True, f"VaR calculations: {len(var_results)}")
            else:
                self.print_result("Monte Carlo API", False, f"Status: {response.status_code}")
                return False
            
            return True
            
        except requests.exceptions.ConnectionError:
            self.print_result("Backend Connection", False, "Cannot connect - is backend running?")
            print("      Start with: python -m uvicorn scripts.simple_main:app --host 0.0.0.0 --port 8002")
            return False
    ...
    def check_frontend(self) -> bool:
        ...
        try:
            ...
            self.print_result("Frontend Dependencies", True, "All required modules available")
        except ImportError as e:
            ...
            if os.path.exists(script):
                ...
                if script.endswith('.sh'):
                    ...
                    if is_executable:
                        ...
                        self.print_result(description, False, "Found but not executable")
    ...
    def generate_summary(self) -> Dict:
        ...
        if not checks["Backend API"]:
            print("- Start backend: python -m uvicorn scripts.simple_main:app --host 0.0.0.0 --port 8002")
