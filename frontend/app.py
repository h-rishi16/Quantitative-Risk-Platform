...
def main():
    """Main application"""
    st.markdown('<h1 class="main-header">🏦 Quantitative Risk Modeling Platform</h1>', unsafe_allow_html=True)
    ...
    page = st.sidebar.selectbox(
        "Select Page",
        ["Portfolio Overview", "Risk Dashboard", "Monte Carlo Simulation", "API Testing"]
    )
...
MSFT,0.30"""
    ...
    with sample_col2:
        ...
        try:
            with open('/Users/hrishi/Projects/Risk Model/sample_data/historical_returns.csv', 'r') as f:
    ...
    if portfolio_file and returns_file:
        ...
        with col1:
            ...
            time_horizon = st.number_input("Time Horizon (days)", min_value=1, max_value=365, value=1)
            confidence_levels = st.multiselect("Confidence Levels", [0.90, 0.95, 0.99, 0.999], default=[0.95, 0.99])
            
            if st.button("🚀 Run Monte Carlo VaR", type="primary", use_container_width=True):
                if confidence_levels:
                    run_monte_carlo_simulation(portfolio_df, returns_df, num_simulations, time_horizon, confidence_levels)
                else:
        ...
        st.info("📁 Please upload both portfolio weights and historical returns files to proceed.")

def run_monte_carlo_simulation(portfolio_df, returns_df, num_simulations, time_horizon, confidence_levels):
    ...
    try:
        ...
        st.error(f"Connection Error: Could not connect to API. Make sure the backend is running.\nError: {str(e)}")
    except Exception as e:
...
def display_simulation_results():
    ...
    if 'var_results' in st.session_state:
        ...
        fig = go.Figure()
        fig.add_trace(go.Histogram(x=simulated_returns, nbinsx=50, name='Portfolio Returns', opacity=0.7))
...
def api_testing():
    ...
    if st.button("🚀 Test with Sample JSON"):
        try:
            response = requests.post("http://localhost:8002/monte_carlo_var", json=sample_json, timeout=30)
            if response.status_code == 200:
